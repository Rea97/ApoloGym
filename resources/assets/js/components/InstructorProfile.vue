            ...
            <div class="col-sm-6">
                ...
                <div class="panel panel-default">
                    ...
                    <div class="panel-body">
                        ...
                        <div v-if="onEdit" class="form-group" :class="{'has-error': errors.has('about_me')}">
                            <textarea v-model="user.about_me" type="text" class="form-control" name="about_me" data-vv-as="Biografía" rows="6"></textarea>
                            <span v-show="errors.has('about_me')" class="help-block">{{ errors.first('about_me') }}</span>
                        </div>
                        <div v-else>
...
<script>
    ...
    export default {
        ...
        props: ['instructor', 'showErrorAlert', 'alertConfirm'],
        mounted() {
            this.user.id = this.instructor.id;
            //this.setUser();
            this.fetchUser();
        ...
        methods: {
            ...
            fetchUser() {
                let _this = this;
                axios.get(`/api/instructors/${this.user.id}`)
                    .then((response)=>{
                        _this.user = response.data.data.instructor;
                        console.log(response);
                    })
            ...
            updateProfile() {
                axios.put(`/api/profile/instructors/${this.user.id}`, this.user)
